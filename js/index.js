...
const btn = document.getElementById('invia');
 ...
 function entra () {
    let cerca = document.getElementById('ins').value;
    let parola = cerca;
    let carattere;
    let parolaInversa = '';
    ...
    console.log(parolaInversa)
      //se la funzione è vera genera il testo è l'utente non può piu andare avanti
    if (parola == parolaInversa ) {
        document.getElementById("inserisci").style.display = "none";
        document.querySelector(".container").innerHTML = `
               <div class="d-flex flex-column justify-content-center align-items-center">
                   <h1 class="text-danger">Parola palindroma : ricarica la pagina!</h1>
               </div> 
        `;
      //se la funzione è falsa genera il testo  + il bottone per andare avanti
        ...
        <div class="d-flex flex-column justify-content-center align-items-center">
        <h1> Benvenuto! </h1>
       <button type="button" class="btn btn-success"><a class="text-white text-decoration-none fs-4" href="./pari-dispari.html">Gioca</a></button>
      </div> 
    `;
    }
